...
class Trajectory:
    ...
    def _find_all_paths(
        self, current_building, current_floor, dest_building, dest_floor
    ):
        paths = []
        ...
        def get_next_waypoints(current_waypoint, visited):
            ...
            for inter_id, waypoints in self.all_interwaypoint_connections.items():
                for waypoint in waypoints:
                    ...
                    if (
                        waypoint["building"] == current_waypoint["building"]
                        and waypoint["floor"] == current_waypoint["floor"]
                    ):
                        if (
                            waypoint["waypoint"] != current_waypoint["waypoint"]
                            and waypoint not in visited
                        ):
                            next_waypoints.append(waypoint)
                    elif (
                        waypoint["id"] == current_waypoint["id"]
                        and waypoint not in visited
                    ):
                        next_waypoints.append(waypoint)
        ...
        for inter_id, waypoints in self.all_interwaypoint_connections.items():
            for waypoint in waypoints:
                if (
                    waypoint["building"] == current_building
                    and waypoint["floor"] == current_floor
                ):
                    queue.append(
                        (waypoint, [waypoint], defaultdict(int))
                    )  # (current_waypoint, path_so_far, id_count)
        ...
        while queue:
            ...
            if (
                current_waypoint["building"] == dest_building
                and current_waypoint["floor"] == dest_floor
            ):
            ...
            for next_waypoint in next_waypoints:
                ...
                queue.append(
                    (next_waypoint, path_so_far + [next_waypoint], new_id_count)
                )
                ...
                for dest_building, dest_floors in self.all_buildings_data.items():
                    for dest_floor, _ in dest_floors.items():
                        ...
                        paths = self._find_all_paths(
                            current_building, current_floor, dest_building, dest_floor
                        )

                        # Store the result
                        precalculated_paths[(current_building, current_floor)][
                            (dest_building, dest_floor)
                        ] = paths
    ...
    def _precalculated_inter_paths(self):
        ...
        for building, building_data in self.all_buildings_data.items():
            for floor, floor_data in building_data.items():
                ...
                current_M = np.atleast_2d(current_M).astype(float)
                _, Pr = shortest_path(
                    current_M, directed=True, method="FW", return_predecessors=True
                )

                interwaypoints = floor_data.get("interwaypoints")

                interwaypoints_index_in_floor = [
                    interwaypoint.get("index") for interwaypoint in interwaypoints
                ]

                anchor_location = self._form_anchor_points(
                    floor_data.get("destinations"), floor_data.get("waypoints")
                )

                for current_index in interwaypoints_index_in_floor:
                    for target_index in interwaypoints_index_in_floor:
                        if current_index < target_index:
                            path_between_interwaypoints[(building, floor)][
                                (current_index, target_index)
                            ] = self._trace_back_path(
                                Pr, anchor_location, current_index, target_index
                            )
                            path_between_interwaypoints[(building, floor)][
    ...
    def _distance(self, c, d, current_bounderies):
        for boundary in current_bounderies:
            ...
            if self._ccw(a, c, d) != self._ccw(b, c, d) and self._ccw(
                a, b, c
            ) != self._ccw(a, b, d):
                return 0
        return np.linalg.norm(np.array(c) - np.array(d))

    def _trace_back_path(
        self, predecessors, locations, current_index, destination_index
    ):
        """
    ...
    def update_destination_graph(self, session_id, navigation_states):
        ...
        floor_data = self.all_buildings_data.get(
            session_data["destination_building"], {}
        ).get(session_data["destination_floor"], {})
        destination_M = floor_data.get("access_graph")
        destination_interwaypoints = floor_data.get("interwaypoints")

        destination_anchor_location = self._form_anchor_points(
            floor_data.get("destinations"), floor_data.get("waypoints")
        )
        ...
        if destination_info:
            for idx, dest_id in enumerate(floor_data.get("destinations").keys()):
                if dest_id == selected_destination_ID:
                    ...
                    session_data["destination_name"] = (
                        floor_data.get("destinations", {})
                        .get(dest_id, {})
        ...
        _, Pr = shortest_path(
            destination_M, directed=True, method="FW", return_predecessors=True
        )

        for interwaypoint in destination_interwaypoints:
            session_data["dest_from_inter"][interwaypoint["index"]] = (
                self._trace_back_path(
                    Pr,
                    destination_anchor_location,
                    interwaypoint["index"],
                    session_data["destination_index"],
                )
            )
    ...
    def calculate_path(self, manager, session_id, localization_states):
        ...
        floor_data = self.all_buildings_data.get(current_building, {}).get(
            current_floor, {}
        )
        current_boundaries = floor_data.get("boundaries")
        current_M = floor_data.get("access_graph")
        current_M = np.atleast_2d(current_M).astype(float)
        current_anchor_location = self._form_anchor_points(
            floor_data.get("destinations"), floor_data.get("waypoints")
        )
        ...
        if (current_building, current_floor) == (
            session_data["destination_building"],
            session_data["destination_floor"],
        ) and session_data["destination_index"]:
            trajectory[0] = {
                "name": "destination",
                "building": current_building,
                "floor": current_floor,
                "paths": [[current_pose[0], current_pose[1]]]
                + self._trace_back_path(
                    Pr, current_anchor_location, -1, session_data["destination_index"]
                ),
                "scale": manager.scale_data.get(manager.config["location"]["place"], {})
                .get(current_building, {})
                .get(current_floor, None),
            }
        else:
            possible_paths = self.precalculated_global_paths.get(
                (current_building, current_floor), {}
            ).get(
                (
                    session_data["destination_building"],
                    session_data["destination_floor"],
                ...
                for step, step_info in enumerate(path_candidate):
                    ...
                    if step == 0:
                        current_paths = [
                            ...
                            Pr, current_anchor_location, -1, step_info.get("index")
                        )
                        distance += self._calculate_trajectory_length(current_paths)
                    elif step == len(path_candidate) - 1:
                        current_paths = session_data["dest_from_inter"].get(
                            step_info.get("index")
                        )
                        distance += self._calculate_trajectory_length(current_paths)
                    else:
                        ...
                        if inter_id not in enter_id:
                            ...
                            current_paths = self.precalculated_inter_paths.get(
                                (building, floor)
                            ).get((step_info.get("index"), target_index))
                            distance += self._calculate_trajectory_length(current_paths)
                        else:
                            continue
                    local_trajectory[step] = {
                        "name": (
                            inter_name
                            if step < len(path_candidate) - 1
                            else session_data["destination_name"]
                        ),
                        "building": building,
                        "floor": floor,
                        "paths": current_paths,
                        "scale": manager.scale_data.get(
                            manager.config["location"]["place"], {}
                        )
                        .get(building, {})
                        .get(floor, None),
                    }
